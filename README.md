...
The long-context gap is entirely the attention mask: `mlx-embeddings` builds the local band as a dense `[L, L]` array, which is O(L²) and costs 10.6 GB at 32k, so throughput collapses as the document grows. Tiling the band keeps it flat instead. The remaining difference at 2k is much smaller and is not isolated here; one plausible contributor is that this implementation keeps the checkpoint's fused gate/up expert projection as a single grouped matmul, where upstream splits it into two. Either way it is ordinary engineering, not a difference in the model.
...
- **Mixture of experts.** 128 experts, top-4 per token, routed by a float32 gate. Routed tokens are sorted by expert id before `mx.gather_mm`, so each expert runs as one grouped matmul over its whole caseload instead of one matmul per routed token. Neither the sorting nor the gather is novel — `mlx_lm.models.switch_layers.SwitchGLU` does both — but the second one is easy to get wrong and expensive when you do: letting `gather_mm` index the rows itself through `lhs_indices` costs 24.2 ms per layer on an 8k-token document, against 5.9 ms for an explicit `t[rows]` gather followed by a plain `gather_mm`. That one line was worth more than twice the end-to-end throughput here.
